...
__kernel void mult_dn(__global cfloat_t* input,
  ...
  float dnDiff = -unit_k*dn[i+Nx*j+offset];
...
__kernel void mult_dn_complex(__global cfloat_t* input,
  ...
  cfloat_t dnDiff = cfloat_mul(cfloat_new(0.f,-unit_k),dn[i+Nx*j+offset]);
...
__kernel void mult_dn_image_complex(__global cfloat_t* plane,
  ...
  cfloat_t dnDiff = cfloat_mul(cfloat_new(0.f,-unit_k),cfloat_new(dn_val.x,dn_val.y));
